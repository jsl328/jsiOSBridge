...
@interface FoxFileManager()

@property (strong, nonatomic) NSFileManager *manager;

@end

@implementation FoxFileManager
...
#pragma mark - 获取文件内容
// 获取文件内容
+ (NSString *)getFileAtPath:(NSString *)path encoding:(NSStringEncoding)encoding {
    ...
    while (! endOfStreamReached)
    {
        ...
        } else if (bytesRead == -1)
        {
            ...
            if (bytesRead) {
                NSString *deString = [[NSString alloc] initWithData:[NSData dataWithBytes:readBuffer length:bytesRead] encoding:encoding];
                if (deString != nil) {
                    [readString appendString:deString];
                }
            }
        }
    }
