...
/** Represents the results of executing a query on an `<YXFMDatabase>`.
...
@interface YXFMResultSet : NSObject {
    YXFMDatabase          *_parentDB;
    YXFMStatement         *_statement;
...
/** `YXFMStatement` used by result set. */

@property (atomic, retain) YXFMStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<YXFMStatement>`
 
 @param statement A `<YXFMStatement>` to be performed
 
 @param aDB A `<YXFMDatabase>` to be used
 
 @return A `YXFMResultSet` on success; `nil` on failure
 */
