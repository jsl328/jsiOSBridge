...
@implementation YXFMResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(YXFMStatement *)statement usingParentDatabase:(YXFMDatabase*)aDB {
    
    YXFMResultSet *rs = [[YXFMResultSet alloc] init];
    ...
    return YXFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    YXFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return YXFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"YXFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (void)setParentDB:(YXFMDatabase *)newDb {
    _parentDB = newDb;
}
