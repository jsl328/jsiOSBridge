...
@interface YXFMDatabasePool()

- (void)pushDatabaseBackInPool:(YXFMDatabase*)db;
- (YXFMDatabase*)db;

@end


@implementation YXFMDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return YXFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"YXFMdb.%@", self] UTF8String], NULL);
        _databaseInPool     = YXFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = YXFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    YXFMDBRelease(_path);
    YXFMDBRelease(_databaseInPool);
    YXFMDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        YXFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(YXFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The YXFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (YXFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [YXFMDatabase databaseWithPath:self->_path];
